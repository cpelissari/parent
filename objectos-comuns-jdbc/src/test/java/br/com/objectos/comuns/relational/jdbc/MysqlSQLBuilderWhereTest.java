...
public class MysqlSQLBuilderWhereTest {
  ...
  public void reset() {
    ...
    sql = sqlProvider.get();
    sql.select("*").from("COMUNS_RELATIONAL.SIMPLE").as("S").andLoadWith(entityLoader);
    sql.order("ID").ascending();
  }

  public void null_condition_should_be_disabled() {
    sql.where("STRING").equalTo(null);
    ...
    assertThat(result.get(2).toString(), equalTo("Simple{id=3, string=ABC}"));
  }
