...
public class MysqlSQLBuilderSelectTest {
  ...
  private final ResultSetLoader<Simple> loader = new SimpleEntityLoader();
  ...
  public void select_columns_should_return_records() {
    ...
    sql.select("*").from("COMUNS_RELATIONAL.SIMPLE").as("S").andLoadWith(loader);
  ...
  public void select_with_explicit_column_names_should_be_ok() {
    Sql sql = sqlProvider.get();
    sql.select("ID", "STRING").from("COMUNS_RELATIONAL.SIMPLE").as("S").andLoadWith(loader);
    sql.order("ID").ascending();
  ...
  public void sum_aggregate_should_be_ok() {
    Sql sql = sqlProvider.get();
    sql.select("sum(ID)").from("COMUNS_RELATIONAL.SIMPLE").as("S").andLoadWith(new LongLoader());
