...
abstract class AbstractJdbcSQLFunction implements HasSQLFunctions, HasElements {
  ...
  public final Join join(String relationship, String alias) {
    Join join = newJoin(JoinType.INNER, relationship, alias);
  ...
  public final Join leftJoin(String relationship, String alias) {
    Join join = newJoin(JoinType.LEFT, relationship, alias);
    elements.put(Join.class, join);
    return join;
  }
  ...
  protected WhereProperty newWhereProperty(String property) {
    return new AnsiWhereProperty(tableAlias, property);
  }

  Multimap<Class<?>, Element> copy() {
    return LinkedHashMultimap.create(elements);
  }
